    ...
    if (trade instanceof LegacyTrade) {
      ...
      result = swapMethods.map(({ methodName, args, value }) => {
        if (isAmbireWC && tokenContract && approvalState === ApprovalState.NOT_APPROVED) {
          ...
          return {
            address: AddressZero,
            calldata: ethers.utils.hexlify(ethers.utils.toUtf8Bytes(JSON.stringify({ batchTxs }))),
            value: '0x0',
          }
      ...
      callback: async function onSwap(): Promise<string> {
        ...
        if (!useSushiGuard) {
          if (
            ...
            const arr = ethers.utils.arrayify(calldata)
            const decoded = new TextDecoder('utf-8').decode(arr)
            const { batchTxs } = JSON.parse(decoded)
