...
import { calculateGasMargin, getArcherRouterContract, getRouterAddress, getRouterContract, isAddress, shortenAddress } from '../utils'
...
import { useUserArcherETHTip, useUserUseArcher } from '../state/user/hooks'
import { ArcherRouter, ArcherSwapParameters } from '../utils/archerRouter'
...
export function useSwapCallArguments(
    ...
    return useMemo(() => {
        ...
        const contract: Contract | null =
            tradeVersion === Version.v2 ? 
                (useArcher ? getArcherRouterContract(chainId, library, account) : getRouterContract(chainId, library, account)) 
                : v1Exchange
        if (!contract) {
            return []
        }
    ...
    }, [account, allowedSlippage, chainId, deadline, library, recipient, trade, v1Exchange, useArcher, archerETHTip, router])
}
...
export function useSwapCallback(
    ...
    const swapCalls = useSwapCallArguments(trade, allowedSlippage, recipientAddressOrName, useArcher)
                ...
                if (useArcher) {
                    ...
                    return contract.signer.populateTransaction(partialTx)
                        ...
                        .then(({signedTx, fullTx}) => {
                            ...
                            const archer = useArcher && archerRelayDeadline ? { 
    ...
    }, [trade, library, account, chainId, recipient, recipientAddressOrName, swapCalls, addTransaction, archerRelayDeadline, archerETHTip])
}
