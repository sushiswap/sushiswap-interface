...
import { signMasterContractApproval } from 'kashi/entities'
...
const useInari = () => {
    ...
    const fetchAllowance = useCallback(async () => {
        if (account) {
            try {
                ...
                console.log('sushiAllowance', sushiAllowance)
    ...
    const fetchAXSushiAllowance = useCallback(async () => {
        if (account) {
            try {
                ...
                console.log('aXSushiAllowance', aXSushiAllowance)
    ...
    const fetchBentoAllowance = useCallback(async () => {
        if (account) {
            try {
                ...
                console.log('bentoAllowance', bentoAllowance)
    ...
    const fetchCrXSushiAllowance = useCallback(async () => {
        if (account) {
            try {
                ...
                console.log('crXSushiAllowance', crXSushiAllowance)
    ...
    const unstakeSushiFromCreamFromBento = useCallback(
        async (amount: BalanceProps | undefined) => {
            console.log('------------sdfjdlkfjlsfl jljl ')
            if (amount?.value) {
                try {
                    console.log('BENTO → CREAM → xSUSHI → SUSHI')
    ...
    const stakeSushiToCream = useCallback(
        async (amount: BalanceProps | undefined) => {
            if (amount?.value) {
                try {
                    const tx = await inariContract?.stakeSushiToCream(account, amount?.value)
