...
const useFarms = () => {
  ...
  const fetchAllFarms = useCallback(async () => {
    ...
    const pairsQuery = results[5].filter((pair) =>
      pairAddresses.includes(pair.id)
    );
    ...
    const farms = pools
      ...
      .map((pool) => {
        if (KASHI_PAIRS.includes(Number(pool?.id))) {
          ...
          const balance =
            Number(pool.slpBalance / 1e18) > 0
              ? Number(pool.slpBalance / 1e18)
              : 0.1;
          ...
          const rewardPerBlock =
            ((pool.allocPoint / masterchefQuery.totalAllocPoint) *
              masterchefQuery.sushiPerBlock) /
            1e18;
    ...
    if (account) {
      const userFarmDetails = await boringHelperContract?.pollPools(
        account,
        pids
      );
      ...
      const userFarms = userFarmDetails
        .filter((farm: any) => {
          return (
            farm.balance.gt(BigNumber.from(0)) ||
            farm.pending.gt(BigNumber.from(0))
          );
        })
        .map((farm: any) => {
          ...
          if (farmDetails && farmDetails.type === "KMP") {
            ...
            depositedUSD =
              farmDetails.totalAssetStaked && farmDetails.totalAssetStaked > 0
                ? (Number(deposited) * Number(farmDetails.tvl)) /
                  farmDetails.totalAssetStaked
                : 0;
          } else {
            deposited = Fraction.from(
              farm.balance,
            ...
            depositedUSD =
              farmDetails.slpBalance &&
              Number(farmDetails.slpBalance / 1e18) > 0
                ? (Number(deposited) * Number(farmDetails.tvl)) /
                  (farmDetails.slpBalance / 1e18)
                : 0;
          }
          const pending = Fraction.from(
            farm.pending,
            BigNumber.from(10).pow(18)
          ).toString(18);
