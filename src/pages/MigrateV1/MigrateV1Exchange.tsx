...
import { Currency, CurrencyAmount, Fraction, JSBI, Percent, Token, TokenAmount, WETH } from '@sushiswap/sdk'
import React, { useCallback, useMemo, useState } from 'react'
import ReactGA from 'react-ga'
import { Redirect, RouteComponentProps } from 'react-router'
import { Text } from 'rebass'
import { ButtonConfirmed } from '../../components/ButtonLegacy'
import { LightCard, PinkCard, YellowCard } from '../../components/CardLegacy'
...
function V1PairMigration({ liquidityTokenAmount, token }: { liquidityTokenAmount: TokenAmount; token: Token }) {
    ...
    const exchangeETHBalance = useETHBalances([liquidityTokenAmount.token.address])?.[
    ...
    return (
        <AutoColumn gap="20px">
            ...
            {!isFirstLiquidityProvider && largePriceDifference ? (
                <YellowCard>
                    <TYPE.body style={{ marginBottom: 8, fontWeight: 400 }}>
                        It{"'"}s best to deposit liquidity into Uniswap V2 at a price you believe is correct. If the V2
                        price seems incorrect, you can either make a swap to move the price or wait for someone else to
                        do so.
                    </TYPE.body>
                    <AutoColumn gap="8px">
                        <RowBetween>
            ...
            {isFirstLiquidityProvider && (
                <PinkCard>
                    <TYPE.body style={{ marginBottom: 8, fontWeight: 400 }}>
                        You are the first liquidity provider for this pair on Uniswap V2. Your liquidity will be
                        migrated at the current V1 price. Your transaction cost also includes the gas to create the
                        pool.
                    </TYPE.body>
        ...
        <AppBody style={{ padding: 24 }}>
            <AutoColumn gap="16px">
                ...
                {!account ? (
                    ...
                    <>
                        <TYPE.body my={9} style={{ fontWeight: 400 }}>
                            Because Uniswap V2 uses WETH under the hood, your Uniswap V1 WETH/ETH liquidity cannot be
