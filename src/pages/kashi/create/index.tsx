...
function Create() {
  ...
  const [selectedAsset, setSelectedAsset] = useState<WrappedTokenInfo | undefined>(undefined)

  const [selectedCollateral, setSelectedCollateral] = useState<WrappedTokenInfo | undefined>(undefined)
      ...
      >
        <div className="space-y-6">
          ...
          >
            {({ open }) => (
              <>
                <Listbox.Label className="block pb-2 text-base font-medium leading-5 text-gray-700">LONG</Listbox.Label>
                  ...
                  <span className="inline-block w-full rounded-md shadow-sm">
                    <Listbox.Button className="relative w-full p-3 text-left transition duration-150 ease-in-out border border-none rounded-md cursor-pointer bg-dark-700 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 sm:text-sm sm:leading-5">
                      <span className="flex items-center space-x-3 truncate">
                        <CurrencyLogo currency={selectedAsset} squared size={48} />
                        <span>
                          <span className="text-lg">
                            {selectedAsset && selectedAsset.symbol}
                            &nbsp;
                          </span>
                          <span className="text-lg text-secondary">{selectedAsset && selectedAsset.name}</span>
                        </span>
                      </span>
                      <span className="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
                        <svg className="w-5 h-5 text-secondary" viewBox="0 0 20 20" fill="none" stroke="currentColor">
                          <path
                    ...
                    >
                      {Object.values(tokens).map((token: any) => (
                        <Listbox.Option key={token.address} value={token}>
                          {({ selected, active }) => (
                            <div className={`${selected || active ? 'bg-dark-blue' : ''} cursor-pointer relative p-3`}>
                              <span className="flex items-center space-x-3 truncate">
                                <CurrencyLogo currency={token} squared size={48} />
                                <span>
                                  <span className="text-lg">
                                    {token.symbol}
                                    &nbsp;
              ...
              <>
                ...
                <div className="relative">
                  <span className="inline-block w-full rounded-md shadow-sm">
                    <Listbox.Button className="relative w-full p-3 text-left transition duration-150 ease-in-out border border-none rounded-md cursor-pointer bg-dark-700 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 sm:text-sm sm:leading-5">
                      <span className="flex items-center space-x-3 truncate">
                        <CurrencyLogo currency={selectedCollateral} squared size={48} />
                        <span>
                          <span className="text-lg">
                          ...
                          <span className="text-lg text-secondary">
                            {selectedCollateral && selectedCollateral.name}
                          </span>
                        </span>
                      </span>
                      <span className="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
