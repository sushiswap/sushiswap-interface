...
interface CrXSushiWithdrawalPanelProps {
    label?: string
...
export default function CrXSushiWithdrawalPanel({
    label = 'Input',
    lpTokenAddress,
    ...
    cornerRadiusTopNone,
}: CrXSushiWithdrawalPanelProps) {
    const { t } = useTranslation()
    const { account } = useActiveWeb3React()
    const theme = useTheme()

    const { crXSushiAllowance, approveCrXSushi, unstakeSushiFromCream } = useInari()

    const tokenBalanceBigInt = useTokenBalance('0x228619cca194fbe3ebeb2f835ec1ea5080dafbb2')
    ...
    const handleApprove = useCallback(async () => {
        try {
            ...
            const txHash = await approveCrXSushi()
    ...
    }, [approveCrXSushi, setRequestedApproval])
            ...
            <InputPanel id={id}>
                ...
                >
                    ...
                    >
                        ...
                        {!crXSushiAllowance || Number(crXSushiAllowance) === 0 ? (
                            ...
                            <ButtonSelect
                                ...
                                onClick={async () => {
                                    setPendingTx(true)
                                    if (maxSelected) {
                                        await unstakeSushiFromCream(maxDepositAmountInput)
                                    } else {
                                        await unstakeSushiFromCream(formatToBalance(depositValue, decimals))
                                    }
                                    setPendingTx(false)
                                }}
                            >
