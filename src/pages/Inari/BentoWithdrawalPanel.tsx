...
interface BentoWithdrawalPanelProps {
    label?: string
...
export default function BentoWithdrawalPanel({
    label = 'Input',
    lpTokenAddress,
    ...
    tokenAddress,
    functionText,
}: BentoWithdrawalPanelProps) {
    const { t } = useTranslation()
    const { account } = useActiveWeb3React()
    const theme = useTheme()

    const { bentoAllowance, approveBento, unstakeSushiFromBento, unstakeSushiFromAave, unstakeSushiFromCreamFromBento, unstakeSushiFromCream } = useInari()
    ...
    const handleApprove = useCallback(async () => {
        try {
            ...
            const txHash = await approveBento()
    ...
    }, [approveBento, setRequestedApproval])
