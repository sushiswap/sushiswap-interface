...
interface AxsushiWithdrawalPanelProps {
    label?: string
...
export default function AxsushiWithdrawalPanel({
    label = 'Input',
    lpTokenAddress,
    ...
    cornerRadiusTopNone,
}: AxsushiWithdrawalPanelProps) {
    const { t } = useTranslation()
    const { account } = useActiveWeb3React()
    const theme = useTheme()

    const { aXSushiAllowance, approveAXSushi, unstakeSushiFromAave } = useInari()

    const tokenBalanceBigInt = useTokenBalance('0xF256CC7847E919FAc9B808cC216cAc87CCF2f47a')
    ...
    const handleApprove = useCallback(async () => {
        try {
            ...
            const txHash = await approveAXSushi()
    ...
    }, [approveAXSushi, setRequestedApproval])
            ...
            <InputPanel id={id}>
                ...
                >
                    ...
                    >
                        ...
                        {!aXSushiAllowance || Number(aXSushiAllowance) === 0 ? (
                            ...
                            <ButtonSelect
                                ...
                                onClick={async () => {
                                    setPendingTx(true)
                                    if (maxSelected) {
                                        await unstakeSushiFromAave(maxDepositAmountInput)
                                    } else {
                                        await unstakeSushiFromAave(formatToBalance(depositValue, decimals))
                                    }
                                    setPendingTx(false)
                                }}
                            >
