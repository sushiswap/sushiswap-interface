...
function MisoInfo({
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      ...
      let remainingTime = ""
      if(days > 0)
        remainingTime += displayDays + 'd '
      if(remainingTime || hours > 0)
        remainingTime += displayHours + 'h '
      if(remainingTime || minutes > 0)
        remainingTime += displayMinutes + 'm '
      if(remainingTime || seconds > 0)
        remainingTime += displaySeconds + 's '
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex flex-row items-end mt-6">
        ...
        <div className="py-1 px-5" style={{borderRadius: 10, backgroundColor: '#809090a0', fontSize: 20, color: 'white', fontWeight: 'bold'}}>
          {symbol}
        </div>
      </div>
      ...
      <div className="flex flex-col mt-6">
        <div>About</div>
        <div>The Trident NFT is introduced as a celebratory piece for the announcement and upcoming release of Sushi’s Trident AMM. This NFT can be redeemed for a 19cm x 19cm 900 tab piece of blotter paper with this Chewy Stoll artwork on the left printed on it.</div>
      </div>
    </div>
  )
}
