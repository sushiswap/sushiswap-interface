...
export default function OnsenTable({ farms, onsenView }: any) {
    ...
    function Percent(props: {number: number}) {
        if (props.number > 0) {
            return (
                <div className="text-green font-normal">+{formattedPercent(props.number)}</div>
            )
        } else {
            return (
                <div className="text-red font-normal">{formattedPercent(props.number)}</div>
            )
        }
    }
    ...
    const FarmRow = ({farm, index}: any) => {
        ...
        return (
            <GridDiv className={"py-4 mb-3 gap-2 text-caption font-bold items-center w-min min-w-full rounded "+bgColor}>
