  ...
  function map(pool, chef) {
    ...
    function getRewards() {
      ...
      if (chef === Chef.MASTERCHEF_V2) {
        ...
        const sushiPerSecond =
          ((pool.allocPoint / 1000) * pool.miniChef.sushiPerSecond) / 1e18;
        const sushiPerBlock = sushiPerSecond * averageBlockTime;
        const sushiPerDay = sushiPerBlock * blocksPerDay;
        const maticPerSecond =
          ((pool.allocPoint / 1000) * pool.rewarder.rewardPerSecond) / 1e18;
    ...
    return {
      ...
      pair: {
        ...pair,
        decimals:
          pair.type === PairType.LENDING ? Number(pair.asset.decimals) : 18,
        type,
      },
      balance,
      roiPerBlock,
  ...
  const positions = farms
    ? [
        ...masterChefV1Positions.map((position) => ({
          ...position,
          ...farms?.find(
            (farm) => farm.id === position.id && farm.chef === Chef.MASTERCHEF
          ),
        })),
        ...masterChefV2Positions.map((position) => ({
          ...
          ...farms?.find(
            (farm) =>
              farm.id === position.id && farm.chef === Chef.MASTERCHEF_V2
          ),
        })),
        ...miniChefPositions.map((position) => ({
          ...position,
          ...farms?.find(
            (farm) => farm.id === position.id && farm.chef === Chef.MINICHEF
          ),
        })),
      ]
    : [];
