...
import { Alert, Button, MovingDots } from 'kashi/components'
...
export default function LendWithdrawAction({ pair }: any): JSX.Element {
    ...
    const displayValue = pinMax
        ? max.toFixed(pair.asset.decimals)
        : value
    ...
    return (
        <>
            ...
            <SmartNumberInput
                ...
                max={max}
                pinMax={pinMax} setPinMax={setPinMax}
                showMax={true}
            />
            ...
            <KashiApproveButton color="blue" content={(onCook: any) => (
                <Button onClick={() => onCook(pair, onExecute)} disabled={displayValue.toBigNumber(pair.asset.decimals).lte(0) || warnings.broken}>
                    Withdraw
                </Button>
            )} />
        </>
