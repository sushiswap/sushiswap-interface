...
export class KashiCooker {
    ...
    borrow(amount: BigNumber, toBento: boolean, toAddress = ""): KashiCooker {
        this.add(Action.BORROW, defaultAbiCoder.encode(['int256', 'address'], [amount, toAddress && toBento ? toAddress : this.account]))
        if (!toBento) {
            const useNative = this.pair.asset.address === WETH[this.chainId].address
    ...
    action(address: string, value: BigNumberish, data: string, useValue1: boolean, useValue2: boolean, returnValues: number) {
        this.add(Action.CALL, defaultAbiCoder.encode(['address', 'bytes', 'bool', 'bool', 'uint8'], [address, data, useValue1, useValue2, returnValues]), value)
    }
