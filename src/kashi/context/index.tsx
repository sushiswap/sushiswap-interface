...
function ChainOracleVerify(pair: any, tokens: any) {
  ...
  if (
    from == pair.assetAddress.toLowerCase() &&
    to == pair.collateralAddress.toLowerCase() &&
  ...
  const updatePairs = useCallback(
    async function() {
      if (boringHelperContract && bentoBoxContract && kashiPairContract) {
        ...
        const pairAddresses = logPairs
          .filter((pair: any) => supported_oracles.indexOf(pair.oracle) != -1 && ChainOracleVerify(pair, tokens))
