    ...
    <div className="flex gap-2 items-center w-[180px]">
      <Menu as="div" className="relative inline-block text-left w-full">
        ...
        >
          <Menu.Items
            static
            className="w-full absolute rounded mt-2 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none border border-dark-900 bg-dark-1000 divide-y divide-dark-900"
          >
            {Object.entries(items).map(([k, v], index) => (
              <div
                key={index}
                onClick={() => setSelected(k as FarmFilter)}
                className="px-3 py-2 hover:bg-dark-900/40 cursor-pointer"
              >
                {v}
              </div>
            ))}
