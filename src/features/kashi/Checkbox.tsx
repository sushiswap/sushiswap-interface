...
export function ExchangeRateCheckBox({ pair, updateOracle, setUpdateOracle, desiredDirection }: any) {
  ...
  const show =
    displayUpdateOracle || desiredDirection === 'up'
      ? pair.oracleExchangeRate.lt(pair.exchangeRate)
      : pair.oracleExchangeRate.gt(pair.exchangeRate)

  return (
    show && (
      <div className="flex items-center mb-4">
        <Checkbox checked={displayUpdateOracle} disabled={pair.exchangeRate.isZero()} set={setUpdateOracle} />
        <span className="ml-2 mr-1 text-primary">Update exchange rate from the oracle</span>
        <QuestionHelper
          text={
            pair.exchangeRate.gt(0)
