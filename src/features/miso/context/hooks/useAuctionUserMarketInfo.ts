import { RawAuctionUserInfo } from 'app/features/miso/context/types'
...
export const useAuctionUserMarketInfos = (addresses: string[], owner?: string): (RawAuctionUserInfo | undefined)[] => {
  ...
  if (results && Array.isArray(results) && results.length === addresses.length) {
    return results.map<RawAuctionUserInfo | undefined>((el) => {
...
export const useAuctionUserMarketInfo = (address: string, owner?: string): RawAuctionUserInfo | undefined => {
  const contract = useMisoHelperContract()
