...
export const useAuctionLiquidityLauncher = () => {
  ...
  const init = useCallback(
    async (data: LiquidityLauncherFormInputFormatted) => {
      if (!contract || !factory || !templateMap || !account || !data.tokenSupply) return

      let templateId
      try {
        templateId = await contract.getTemplateId(templateMap[LiquidityLauncherTemplate.PostAuctionLauncher].address)
        ...
        console.error('Initialize liquidity launcher error: ', e)
        console.log(
          contract.address,
          contract.interface.encodeFunctionData('createLauncher', [
            templateId,
            data.tokenAddress,
            data.tokenSupply.quotient.toString(),
            AddressZero, //feeIntegrator
            defaultAbiCoder.encode(
              ['address', 'address', 'address', 'address', 'uint256', 'uint256'],
              [data.auctionAddress, factory.address, account, data.lpOwnerAddress, data.liqPercentage, data.liqLockTime]
            ),
          ])
        )
      }
