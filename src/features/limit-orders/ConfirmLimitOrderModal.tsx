...
const ConfirmLimitOrderModal: FC<ConfirmLimitOrderModalProps> = ({
  open,
  onDismiss,
  onConfirm,
}) => {
  ...
  const bottomContent = useCallback(
    () => <ConfirmLimitOrderBottomContent onClick={onConfirm} />,
    [onConfirm]
  );
...
const ConfirmLimitOrderTopContent = () => {
  ...
  const inputUSDC = useUSDCPrice(
    currencies[Field.INPUT] && chainId in USDC
      ? currencies[Field.INPUT]
      : undefined
  )?.toFixed(18);
  const inputValueUSDC = formatNumber(
    Number(parsedAmounts[Field.INPUT].toSignificant(6)) * Number(inputUSDC)
  );

  const outputUSDC = useUSDCPrice(
    currencies[Field.OUTPUT] && chainId in USDC
      ? currencies[Field.OUTPUT]
      : undefined
  )?.toFixed(18);
  const outputValueUSDC = formatNumber(
    Number(parsedAmounts[Field.OUTPUT].toSignificant(6)) * Number(outputUSDC)
  );
      ...
      <div className="flex flex-col gap-6">
        <div className="flex flex-col gap-3">
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="text-low-emphesis text-sm">
              ≈ {inputValueUSDC} USDC
            </div>
          </div>
        </div>
        <div className="bg-dark-800 rounded flex justify-between py-3 px-5">
          ...
          <span className="text-primary">
            {limitPrice} {currencies[Field.OUTPUT]?.symbol} per{" "}
            {currencies[Field.INPUT]?.symbol}
          </span>
        </div>
        <div className="flex flex-col gap-3">
...
const ConfirmLimitOrderBottomContent: FC<ConfirmLimitOrderBottomContentProps> =
  ({ onClick }) => {
    const { i18n } = useLingui();
