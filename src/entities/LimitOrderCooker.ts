...
import { getVerifyingContract, LimitOrder } from "limitorderv2-sdk";
...
export default class LimitOrderCooker {
  ...
  bentoDepositCollateral(amount: BigNumber): LimitOrderCooker {
    ...
    this.add(
      ...
      defaultAbiCoder.encode(
        ...
        [
          useNative ? ethers.constants.AddressZero : this.token.address,
          this.account,
          amount,
          0,
        ]
  ...
  async cook() {
    ...
    const limitOrderContract = new Contract(
      getVerifyingContract(this.chainId),
      LIMIT_ORDER_ABI,
    ...
    try {
      return {
        success: true,
        tx: await limitOrderContract.cook(
          this.actions,
          this.values,
          this.datas,
          {
      ...
      console.error("LimitOrderCooker Error: ", error);
      return {
        success: false,
        error: error,
      };
