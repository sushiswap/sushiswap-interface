...
interface NeonSelectProps {
  value;
  children:
    | React.ReactElement<NeonSelectItemProps>
    | React.ReactElement<NeonSelectItemProps>[];
}
  ...
  return (
    <div className="relative" ref={node} onClick={toggle}>
      <div className="shadow-md z-[2] relative flex border border-dark-800 bg-dark-900 h-[38px] rounded-md divide-x divide-dark-800">
        <div className="text-sm text-primary flex items-center pl-3 min-w-[80px] font-medium">
          {value}
        </div>
...
export const NeonSelectItem: FC<NeonSelectItemProps> = ({
  onClick,
  value,
  children,
}) => {
