...
export function KashiProvider({ children }: { children: JSX.Element }) {
  ...
  const updatePairs = useCallback(async () => {
    ...
    if (boringHelperContract && bentoBoxContract) {
      ...
      const balances = rpcToObj(
        await boringHelperContract.getBalances(account, pairAddresses)
      );
  ...
  useEffect(() => {
    if (
      account &&
      chainId &&
      [ChainId.MAINNET, ChainId.KOVAN, ChainId.BSC, ChainId.MATIC].indexOf(
        chainId
      ) != -1
    ) {
      updatePairs();
...
export function useKashiPair(address: string) {
  ...
  return context.state.pairs.find((pair: any) => {
    return (
      ethers.utils.getAddress(pair.address) === ethers.utils.getAddress(address)
    );
  });
}
