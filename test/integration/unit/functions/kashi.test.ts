...
describe('kashi functions', () => {
  const pair = {
    ...
    userBorrowPart: ZERO,
    currentExchangeRate: BigNumber.from('257113025824117000000000000'),
  ...
  describe('accrue', () => {
    it('includes principal', () => {
      ...
      expect(res.toString()).to.equal('167405778425')
    })

    it('does not include principal', () => {
      const res = accrue(pair, pair.totalBorrow.elastic, false)

      expect(res.toString()).to.equal('5236342')
    })
  })
  ...
  describe('interestAccrue', () => {
    it('returns starting interest if totalBorrow is 0', () => {
      ...
      expect(res.eq(STARTING_INTEREST_PER_YEAR)).to.be.true
    })
    ...
    it('handles utilization lower than target', () => {
      ...
      expect(res.toString()).to.equal('51404697414431999')
    })
    ...
    it('handles utilization lower than target and interest lower than minimum', () => {
      ...
      expect(res.eq(MINIMUM_INTEREST_PER_YEAR)).to.be.true
    })
    ...
    it('handles utilization higher than target', () => {
      ...
      expect(res.toString()).to.equal('51404697414432000')
    })
    ...
    it('handles utilization higher than target and higher than maximum', () => {
      ...
      expect(res.eq(MAXIMUM_INTEREST_PER_YEAR)).to.be.true
    })
    ...
    it('returns input interest if utilization is between range', () => {
      ...
      expect(res.eq(interest)).to.be.true
    })
  })
  ...
  describe('getUSDValue', () => {
    it('returns usd value', () => {
      ...
      expect(res.eq(amount)).to.be.true
    })

    it('get top level decimals if available', () => {
      const decimals = 8
      const assetInput = {
        ...pair.asset,
        decimals,
      ...
      expect(res.eq(compare)).to.be.true
    })
  })
  ...
  describe('takeFee', () => {
    it('subtracts fee', () => {
      ...
      expect(res.toString()).to.equal('18')
    })
  })
  ...
  describe('addBorrowFee', () => {
    it('adds borrow fee', () => {
      ...
      expect(res.toString()).to.equal('2001')
    })
  })
